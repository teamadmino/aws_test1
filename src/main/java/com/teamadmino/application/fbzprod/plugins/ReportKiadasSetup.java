...
public class ReportKiadasSetup {
    ...
    public static void init_onload(RequestContext rc) throws Exception {
        ...
        String group = settings.optString("kiadasGeneric_group");
        if (group.length() != 0) {
            rc.setValue("reportType", group);
            JSONArray filters = settings.getJSONArray("kiadasGeneric_filters");
    ...
    public static void addfilter_action(RequestContext rc) throws Exception {
        ...
        for (String item : "Tulajdonos,Vevő,Időszak,Bizonylat száma,Bizonylat éve,Bizonylat jelleg".split(",")) {
            desc.put(item);
        }
    ...
    public static void addfilter_action_onreturn(RequestContext rc) throws Exception {
        switch (rc.getReturnValue().getString("col0")) {
            ...
            case "Vevő": {
                JSONArray kod = new JSONArray().put("id");
                JSONArray name = new JSONArray().put("name");
                DatabaseTable vevo = tables.get("VEVO");
                for (int i = 1; i <= vevo.recordCount; i++) {
                    MemoryRecord memoryRecord = vevo.getMemoryRecord(i, 1);
                    kod.put(memoryRecord.getASFormattedString("kod"));
                    name.put(memoryRecord.getASFormattedString("name"));
                }
                JSONArray data = new JSONArray();
                ...
                data.put(name);
                JSONObject args = new JSONObject().put("data", data);
                rc.loadToPopupTest2("_admino:generic/selectSimple", args, "getVevo");
                break;
            }
            case "Időszak":
            ...
            case "Bizonylat jelleg":
                rc.loadToPopupTest2("fbz_prod:common/getKiadasJelleg", null, "getBizJelleg");
                break;
    ...
    public static void getVevo_popup_onreturn(RequestContext rc) throws Exception {
        rc.closeSender();
        DatabaseTable filterTable = tables.get(rc.getScreenVars().getString("filterTableName"));
        new Record(filterTable)
            .setField("field", "Vevő")
    ...
    public static void run_action(RequestContext rc) throws Exception {
        ...
        settings.put("kiadasGeneric_group", rc.getValueString("reportType"));
        settings.put("kiadasGeneric_filters", filters);
        sessionVars.put(sid, settings);

        int jobId = createJob(new JobConfig("Kiadás")
                                  .putParam("handler", "kiadasGenericV0")
